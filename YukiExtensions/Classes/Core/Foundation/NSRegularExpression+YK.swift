...
public extension Yuki where Base: NSTextCheckingResult {
    /// 返回匹配字符串
    @discardableResult
...
public extension NSRegularExpression {
    static func cd_init(_ pattern: String, options: NSRegularExpression.Options = []) -> NSRegularExpression? {
        return Yuki.makeRegEx(pattern, options:options)
    }
}

public extension Yuki where Base: NSRegularExpression {
    ...
    func match(replaceIn string:String, with template:String, count: Int? = nil, options: NSRegularExpression.MatchingOptions = []) -> String {
        ...
        for item in ranged.reversed() {
            let replacement = item.YK.replace(in: string, template: template)
            if let range = item.YK.range(in: string) {
...
public extension Yuki where Base == String {
    func regex(_ pattern:String) -> Bool {
        return Yuki<NSRegularExpression>.makeRegEx(pattern)?.YK.match(base) ?? false
    }
}
